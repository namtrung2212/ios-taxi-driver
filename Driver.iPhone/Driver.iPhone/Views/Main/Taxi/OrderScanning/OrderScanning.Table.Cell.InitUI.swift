...
open class OrderScanningCell: UITableViewCell, ModelTransfer,GMSMapViewDelegate {
    ...
    func initControls(_ completion: (() -> ())?){
       ...
       // pnlCellArea.layer.borderColor = UIColor.grayColor().CGColor
        ...
        lblDistance = UILabel(frame: CGRect(x: 0, y: 0, width: 0, height: 50))
        lblDistance.font = UIFont(name: "HelveticaNeue-Bold", size: 17)
        lblDistance.textColor = UIColor.darkGray
    ...
    func initLayouts(_ completion: (() -> ())?){
       ...
       let topLine = CAShapeLayer()
        ...
        self.pnlBottomInfoArea.addSubview(lblDistance)
        lblDistance.bottomAnchor.constraint(equalTo: self.lblDateTime.bottomAnchor, constant: 0.0).isActive = true
        lblDistance.rightAnchor.constraint(equalTo: pnlBottomInfoArea.rightAnchor, constant : -10.0).isActive = true
        lblDistance.heightAnchor.constraint(equalToConstant: 15).isActive = true
        lblDistance.widthAnchor.constraint(equalToConstant: 100).isActive = true
