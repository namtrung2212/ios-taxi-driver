...
open class OrderScanningScreen: UIViewController {
    
    var table: OrderScanningTable!
    ...
    func initControls(_ completion: (() -> ())?){
        ...
        let btnBack = UIBarButtonItem(title: "", style: .plain, target: self, action: #selector(OrderScanningScreen.btnBack_Clicked))
        btnBack.setFAIcon(FAType.faArrowLeft, iconSize : 20)
        btnBack.setTitlePositionAdjustment(UIOffsetMake(0, -5), for: .default)
        self.navigationItem.leftBarButtonItem = btnBack
        self.navigationItem.title = "Nhu cầu quanh đây"
        ...
        lblNoTravel.text = "Không có yêu cầu nào."
        lblNoTravel.translatesAutoresizingMaskIntoConstraints = false
        lblNoTravel.isHidden = true
        
        table = OrderScanningTable(parent: self)
        table.initControls{
            self.table.initLayout{
    ...
    open func btnBack_Clicked() {
        ...
        AppDelegate.mainWindow?.taxiViewCtrl.slideMenuController()?.openLeft()
